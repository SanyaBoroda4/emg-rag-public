...
> prior context) fully up to speed. Last updated: 2026-09-21, after WO21 (portfolio polish), WO20 (correctness pass; restart pending), WO19 Part 3 (public repo recreated), WO19 (public mirror audit; Part 3 decision pending), WO18 (dark console restyle; restart pending), WO17 (conversational follow-ups; live) and WO16 (Phase 7: served at https://rag.emgcheckbot.us)
...
| SRV | WO16: Phase 7 — serve over HTTPS (FastAPI + single-page UI + Caddy) | **COMPLETE.** `retrieval/pipeline.py:ask()` extracted (CLI output byte-identical on Q1/Q59; Q27 differs only by temperature-0 wording jitter, chunk lines md5-identical). `sql/020` `serve` schema + `rag_serve` role (fence verified: `rag_reader` denied on `serve`). `serve/app.py`: `/`, `/healthz`, `/api/ask` (500-char cap, 2 in flight → 429, 60 s → 504, one `serve.asks` row per ask incl. errors), `/api/feedback` (Langfuse `user_feedback`), `/api/history` (+`/{id}`, per-user, 404 for Crew R). `serve/static/index.html`: one file, no CDN, evidence tabs, feedback, history sidebar. Unit on **172.18.0.1:8080** (a 127.0.0.1 bind is unreachable from Caddy's bridge). Caddy block + steps in `deploy/`; Alex installed the unit and reloaded Caddy by hand (permission policy blocks Claude Code there) — **bind-mounted Caddyfile: never `sed -i`**. Part 5: 401 without creds, LE cert, five asks = CLI, Langfuse `ui`/`user_id`/session + feedback score, 400/429, memory idle 122 MB / in flight 134 MB / host ≥ 912 MB, containers unchanged, history per user + timeout row. Final eval @ `fd2ae61`: 96.3 / 90.2 (74/82, floor) / 93.6, R@10 0.818, $1.02, 251 s; Q68/Q76 red on SQL-text jitter, Q41 exposes the per-column (not per-view) validator whitelist. Deliverable: `evals/results/wo16_serving.md`. Cost ≈$1.20 |
| CONV | WO17: conversational follow-ups (rewrite, don't re-prompt) | **COMPLETE — live since the restart at `2b9c5fb`.** `retrieval/rewrite.py`: Haiku 4.5 @ temperature 0 decides standalone vs follow-up on the last ≤3 turns (question, resolved question, route, answer ≤600 chars, SQL — never rows); standalone = verbatim by code, no call with empty history or after a refuse, failures fail open. Own `rewrite` span in the `ui` trace. `sql/021` conversation columns; `/api/ask` takes `conversation_id`, history grouped by conversation; UI shows **"Interpreted as: …"** with a Rephrase link, New conversation, conversation sidebar. Router/SQL/answer/judge untouched (`harness.py` diff 0). **Before:** 1/20 follow-ups as typed (16 refused). **After** (3 runs @ `7141016`): follow-ups **15–16/18**, standalone pass-through **20/20 every run**, flag variance 0/38, rewrite $0.0014 / 1.8 s per call. Persistent reds: C5.3 ("was 2023 better?" loses the comparison), C11.2 (ambiguous by design, visible), C15.3 (post-refuse rule), plus the Q68/Q76 SQL jitter. 15 golden conversations / 38 turns all **draft** until Alex verifies (`evals/golden_conversations.csv`); tier = `python evals/run_eval.py --conversation` (~$0.50). Single-question eval 73/82 (Q58 judge coin; path unchanged). Deliverable: `evals/results/wo17_followups.md`. Cost ≈$3.15 |
| UI | WO18: dark console restyle | **COMPLETE — service restart pending (Alex).** `serve/static/index.html` restyled in place: slate tokens on `:root`, one cyan accent, IBM Plex Sans/Mono self-hosted (`serve/static/fonts/`, OFL 1.1, Latin-1 subsets, 78 KB) served by a new `/static/fonts` StaticFiles mount in `serve/app.py` (the only Python change; no API change). Evidence panel open by default for structured/hybrid answers, drawn as a terminal block (`sql ›` prompt, tinted keywords, mono rows right-aligned); numbers in answer prose take the mono face. Exactly two motions (seconds counter, 150 ms card fade; both off under `prefers-reduced-motion`) — the old spinner ring and drawer slide are gone. JS untouched except class names, `details open`, the number/keyword spans, numeric-column alignment and separator text. 12-point behaviour checklist ticked against a stdlib mock of the five endpoints in local Chrome (1280 px and a 380 px iframe; the server has no browser); screenshots in `evals/results/wo18_screens/`. Page weight 17.0 KB → 20.7 KB HTML + 76.5 KB fonts (cached after first load). $0 API. Deliverable: `evals/results/wo18_restyle.md` |
| PUB | WO19: public mirror PII audit, allowlist + guard | **COMPLETE — Part 3 option (a) executed 2026-09-21: old repo renamed `emg-rag-public-archive` (private), new `emg-rag-public` created public, first snapshot `afd5dc3`, full-history scan 0 guarded hits, tests workflow green, badge passing (report §9).** Diagnostic: the old `sync_public.sh` replayed the full private history through filter-repo, so every run JSON, per-question render and WO report (customer names, street addresses, verbatim note text) is readable in the public repo's history: 52 files at HEAD and 71 blob versions with guarded hits, 0 secrets anywhere, 0 history-only paths; the public repo has 1 star, 0 forks and 206 clones by 79 unique cloners in 14 days. Term lists (6,834 customer name forms, 4,413 surname tokens, 56 phones, 2,661 emails, 4,029 addresses, 37 employees → `Salesperson A–K` / `Crew A–Z`, 42,900 note shingles from 7,194 chunks) live in `private/` (gitignored; rebuild with `scripts/build_pii_terms.py` + `build_pii_shingles.py` on the server and `build_pii_exclusions.py` locally, see `docs/wo19_public_audit.md` §1.2). New mirror: `deploy/public_allowlist.txt` → `scripts/public_transform.py` (pseudonyms, `Customer #<job_id>` / `[customer]`, `[redacted]`, `$[redacted]`, `[note text]` windows, IP) → `scripts/public_scan.py` guard (exit 1 = nothing pushed) → one snapshot commit per sync. `tests/test_public_guard.py` (5 tests, fake terms, CI-safe). Dry run @ `47adae8`: 137 files kept, 51 held back, 0 excluded after transform, 213 customer names + 53 tokens + 514 note windows + 584 employee mentions + 23 dollar figures rewritten; scan of the result: 0 customer / contact / note / secret / dollar hits. **Next:** Alex picks Part 3 option (a) recreate the public repo (recommended) or (b) filter-repo, commands prepared in the report; then `bash scripts/sync_public.sh`. Cost $0. |
| FIX | WO20: correctness pass (seven items) | **COMPLETE — service restart pending (Alex).** Q16 key confirmed 4,423 (no change). Rewriter: worked "was X better" example (`abd6564`), refused turns skipped when building history (`e54c2d6`), fixtures re-recorded 21 cases (`7609331`). Answer prompt: notes described by provenance (`a1a5e06`). SQL validator per alias via sqlglot scope (`da5a346`), then fixed to check a column only in its own scope after run 1 falsely rejected Q73's scalar subquery (`a070597`); 116/116 recorded queries accepted, Q41 and Q26-class rejected with column/view/alias named. Chips are buttons (`3132274`). Mirror: two-decimal money without a sign redacted (`0b22ca1`, `015f22a`). Measured: full eval 74/73 @ `1e4a4e6`, **75/75 @ `a070597`** (routing 96.3%, R@10 0.818 / MRR 0.660 unchanged, zero SQL errors, Q41 fails cleanly on its key, Q73 ✓); re-judge ×3 of run 1: Q30 ✓/✓/✓ (74/73/73, Q48/Q52 coins); conversation ×4 @ `a070597`: 34/34/34/35 of 38, C5.3 ✓ C15.3 ✓ C7.2 unchanged, C9.2 ✗ on SQL jitter (identical rewrite, no repair), C15.3 relabelled follow-up in `golden_conversations.csv` (16–17/19 follow-ups, 19/19 standalone byte-identical). Cost ≈ $7.66 (tripwire $8; the validator fix forced a second chain). Deliverable: `evals/results/wo20_correctness.md`; runs `evals/results/2026-09-21-*-wo20-*.json` |
| PORT | WO21: Phase 8 portfolio polish (README, notes, CI, demo, facts) | **COMPLETE — public confirmation waits on WO19 Part 3.** `README.md` rewritten (1,146 words: mock screenshot, Mermaid architecture, results table with a source per row from the WO20 runs @ `a070597`, seven decisions with evidence files, what does not work, cost, limitations); old body → `docs/OPERATING_NOTES.md`. `docs/ENGINEERING_NOTES.md` (one section per WO + lessons), `docs/COST_LEDGER.md` ($28.41 in run files / ≈$35.61 by WO cost sections), `docs/DEMO_SCRIPT.md` (six questions verified on live data 2026-09-21: counts/percentages/areas only; `office` login has no history), `docs/RESUME_FACTS.md`. CI: `.github/workflows/tests.yml` runs the three unit-test files on push with no secrets — **green on the private repo** (`ff465d9`); `eval.yml` is `workflow_dispatch` only (it had failed on every push for lack of secrets). Badge targets the public repo's `tests` workflow and renders once the first allowlisted snapshot is pushed. $0.04. Deliverable: `docs/wo21_portfolio.md` |
...
- **Phase 7 ✅ — the RAG is served at https://rag.emgcheckbot.us** (WO16,
  ...
  - WO17's follow-ups went live when the service was restarted at `2b9c5fb`
    (correction, WO20: the WO18 report said production still served the WO16
    page; it was serving WO17).
  - **⚠ WO18 (2026-09-21) restyled the page (dark console) and added the
    `/static/fonts` route**; WO20 changed the rewriter, the answer prompt, the
    SQL validator and the UI chips — run `systemctl restart emg-rag-api` to
    serve them; until then the page is the WO17 light skin.
  - WO19 Part 3 done 2026-09-21 (option a): the public repo is a fresh
    snapshot repository; the old one is `emg-rag-public-archive`, private.
    `scripts/sync_public.sh` adds one snapshot commit per run; never push to
    the public repo any other way.
  - **Add a golden conversation:** append rows to
    `evals/golden_conversations.csv` (`conv_id, turn, question,
    expected_standalone` = the self-contained question or `SAME`,
    `expected_route, expected_answer, status, notes` with the psql that
    produced the key); keep keys to the numbers the question asks for; run
    `python evals/run_eval.py --conversation`; flip `status` to `verified`
    only after checking the key. If the rewriter prompt changes, re-record
    the unit-test fixtures: `python evals/make_rewrite_fixtures.py`, then
