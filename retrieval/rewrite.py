...
Design rules (see the WO17 report for the measured behaviour):
  ...
  * with no history, no model call is made at all: the question is
    standalone by rule. Refused turns are skipped when building the history
    (WO20; before that a refuse as the previous turn stopped rewriting
    entirely): the remaining on-topic turns, still at most three, are used,
    and if none remain there is no call;
...
- Otherwise return {"standalone": false, "question": "<one self-contained question>", "reason": "..."} that resolves ONLY:
  ...
  * "Was X better?" / "Was X worse?" / "Was X higher?" is ALWAYS a comparative: X is the new value of the previous turn's filter, the other side is the previous turn's own value of that filter, and the metric is the previous turn's metric. Worked example — previous turn resolved to "What was our quote conversion rate in 2024?", NEW question "Was 2023 better?" → {"standalone": false, "question": "Was the quote conversion rate in 2023 better than in 2024?"}. Wrong: "What was the quote conversion rate in 2023?" (that drops the comparison and the 2024 side).
...
def rewrite(question: str, history, *, call=None) -> RewriteResult:
    ...
    # WO20: refused turns are skipped, not stopped at. Off-topic turns never
    # reach the prompt (same safety as before); the on-topic turns around
    # them still do, so "those quiet jobs" two turns back survives a refuse.
    turns = [t for t in (history or [])
             if t is not None and t.route != "refuse"][-MAX_HISTORY:]
    if not turns:
        return RewriteResult(True, question,
                             reason="no history" if not history
                             else "no on-topic history (refused turns are skipped)")
