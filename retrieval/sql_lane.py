...
v_job_sqft(job_id, total_sq_ft numeric, area_count)
  ...
  -- PER JOB vs PER VISIT are DIFFERENT metrics and the question's wording decides which: "per job" / "job size" = SUM(job sq ft) / COUNT(DISTINCT job_id) — the job's sq ft counted ONCE however many visits it took; "per visit" / "per install visit" / "per trip" = SUM(job sq ft) / COUNT(install activity_id) — the same per-job totals divided by the number of happened Install activities. Per-visit pattern (each visit row carries its job's total; the job total is NOT re-summed per area):
  --   SELECT COUNT(*) AS visits, COUNT(DISTINCT a.job_id) AS jobs, SUM(COALESCE(s.total_sq_ft, 0)) / COUNT(*) AS sq_ft_per_visit FROM v_activities a LEFT JOIN v_job_sqft s USING (job_id) WHERE a.type_name = 'Install' AND a.happened AND <your filters>
  --   — this is NOT the per-job total (that would double count): report it only as "per visit". Canonical example: a job of 88.5 sq ft installed over 2 visits is 88.5 sq ft per job and 44.25 sq ft per visit. When a question asks for both visits and jobs, select both counts in one query and report both numbers.
