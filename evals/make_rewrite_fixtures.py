"""Record the rewriter's outputs for 21 fixed cases (WO17; F11 added in WO20)
so the unit tests can run without an LLM.
...
through byte-identical) and 11 follow-ups (must resolve). History turns are
...
def cases(turns):
    ...
    standalone = [
        ...
        ("S07", "How many jobs did Crew J install in the first half of 2026?", T(("C10", 1))),
    ...
    followups = [
        ...
        ("F10", ("C14", 2), T(("C14", 1))),
        # WO20: the "was X better?" comparative that C5.3 kept losing
        ("F11", ("C5", 3), T(("C5", 1), ("C5", 2))),
    ]
    out = []
